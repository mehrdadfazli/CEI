...
# Usage: ./run_chair.sh [config_file]
...
LOG_DIR=$(jq -r '.chair_log_dir' "$CONFIG_FILE")
...
OPERA_RESULTS=$(jq -r '.opera_results' "$CONFIG_FILE")
INJECTION_LAYER=$(jq -r '.injection_layer' "$CONFIG_FILE")
ALPHA=$(jq -r '.alpha' "$CONFIG_FILE")

# Validate required fields
if [ -z "$MODEL_TYPE" ] || [ -z "$LOG_DIR" ] || [ -z "$NUM_BEAMS" ] || [ -z "$MAX_NEW_TOKENS" ] || \
   [ -z "$CONTEXT_STRATEGY" ] || [ -z "$TOPK" ] || [ -z "$INJECTION_LAYER" ] || [ -z "$ALPHA" ]; then
...
ARGS+=" --topk $TOPK"
ARGS+=" --opera_results $OPERA_RESULTS"
ARGS+=" --injection_layer $INJECTION_LAYER"
ARGS+=" --alpha $ALPHA"

# Run the Python script
echo "Running run_CHAIR_dyn.py with arguments:$ARGS"
python3 run_CHAIR_dyn.py $ARGS
