#!/usr/bin/env bash
set -euo pipefail
# Usage: ./scripts/run_pope.sh <config.json> <log_dir> <question_dir> <image_folder> [cache_dir]

CONFIG_FILE="${1:?config.json}"
LOG_DIR="${2:?log_dir}"
QUESTION_DIR="${3:?POPE questions dir}"
IMAGE_FOLDER="${4:?COCO val2014 images}"
CACHE_DIR="${5:-}"
...
cmd="python run_POPE.py --config \"$CONFIG_FILE\" --question_dir \"$QUESTION_DIR\" --image_folder \"$IMAGE_FOLDER\" --log_dir \"$LOG_DIR\""
cmd="$cmd --max_new_tokens ${max_new_tokens:-32} --repetition_penalty ${repetition_penalty:-1.0}"
...
cmd="$cmd --alpha_method ${alpha_method:-cosine} --alpha ${alpha:-0.1}"
cmd="$cmd --tau ${tau:-0.2} --T ${Tval:-0.05} --beta ${beta:-0.3}"
cmd="$cmd --delta ${delta:-0.3} --gamma ${gamma:-0.2} --dynamic_mode ${dynamic_mode:-two_pass}"
