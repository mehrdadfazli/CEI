...
def write_summary_file(path, cap_file, caption_key, cap_dict):
    ...
    out = {
        ...
        "metrics_fraction": {str(k): float(v) for k, v in metrics.items()},
        "metrics_percent": {str(k): round(float(v) * 100, 4) for k, v in metrics.items()},
    }
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional JSON path with cap_file, caption_key, and CHAIR metrics (fraction and percent).",
    )
    ...
    if args.summary_file:
        write_summary_file(args.summary_file, args.cap_file, args.caption_key, cap_dict)
